...
try:
    ...
    if args.verbose:
        ...
        p.print_trx_results(tresults)
        print(f'Fee to run trx = {Convert().from_wei(trx_fee, "gwei")} gwei.')
        print('')
...
try:
    trx_name = 'storeNums'
    max_priority_fee=int(constant.MAX_PRIORITY_FEE_WEI / 2)
    max_fee=int(constant.MAX_FEE_WEI / 2)
    max_priority_fee_gwei = Convert().from_wei(max_priority_fee, "gwei")
    max_fee_gwei = Convert().from_wei(max_fee, "gwei")
    initial_balance = b.get_balance(user)
    tresults = c.run_trx(
        user,
        trx_name,
...
try:
    ...
    pass_count += 1
    if args.verbose:
        print(f'PASS:  Unable to get past events for "{event_name}" event name.')
        print(f'MESSAGE:  {e}')
        print('')
else:
    fail_count += 1
...
try:
    ...
    print(
        ...
        f'blocks while using wrong type for event_name: {ename}. Should not happen.'
        )
    p.print_event_list(elist, indent=4)
    print('')
...
try:
    ...
    if args.verbose:
        ...
        p.print_trx_results(tresults)
        print(f'Fee to send trx = {Convert().from_wei(trx_fee, "gwei")} gwei.')
        print('')
