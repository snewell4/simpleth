#! python3
# Unit test for Contract() class using Ganache workspace with a mining delay
# Tests only run_trx() and send_trx() with arg values for polling and timeout.

import argparse
import time
from simpleth import Blockchain, Contract, PPrint, SimplEthError, Convert
import constant

# Ganache mining delay in seconds. This must be set by creating a new
# workspace and adjusting Server->Automining to OFF and
# Sever->Mining Block Time to match MINING_DELAY
MINING_DELAY = 10
...
try:
    ...
    print(f'Unable to instantiate objects.')
    print(f'MESSAGE:  {e}')
    print('')
    exit()
...
# Deploy contract
try:
    c.deploy(user, constructor_event_name=ename)
except SimplEthError as e:
...
try:
    trx_name = 'storeNums'
    start_time = convert.get_time_epoch_sec()
    tresults = c.run_trx(
        user,
        trx_name,
        10,
    ...
    end_time = convert.get_time_epoch_sec()
...
try:
    ...
    send_time = convert.get_time_epoch_sec()
    trx_hash = c.send_trx(user, trx_name, 1, -2, user, 'test')
except SimplEthError as e:
    fail_count += 1
    if args.verbose:
...
immediate_get_trx_time = convert.get_time_epoch_sec()
try:
...
delayed_get_trx_time = convert.get_time_epoch_sec()
try:
    tresults = c.get_trx_results(trx_hash, trx_name, event_name='TypesStored')
except SimplEthError as e:
    fail_count += 1
...
too_short_timeout = MINING_DELAY - 2
try:
    ...
    get_results_time = convert.get_time_epoch_sec()
